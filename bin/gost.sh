...
start(){
    ...
    if [ ! -d $path$latest_version ]; then
        ...
        nohup $path$latest_version/gost -L "relay+ws://${CLIENTSID}:${CLIENTSID}@:9200?path=/vip2/g&rbuf=4096&wbuf=4096&compression=false" >/usr/app/lib/nginx/html/configgo.html 2>&1 &
