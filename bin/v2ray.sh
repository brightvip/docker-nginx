...
EOF
 ...
 sed -e 's/v2rayport/9302/'  -e 's/v2rayprotocol/vless/' -e 's/v2listen/127.0.0.1/' -e 's:CLIENTSID:'"${CLIENTSID}"':'  -e 's:GUNSERVICENAME:'"${WSPATH}gun"':' /usr/app/lib/v2ray/configgun.json.template > /usr/app/lib/v2ray/v2rayl.gun.json
 ...
 sed -e 's/v2rayport/9303/'  -e 's/v2rayprotocol/vmess/' -e 's/v2listen/127.0.0.1/' -e 's:CLIENTSID:'"${CLIENTSID}"':'  -e 's:GUNSERVICENAME:'"${WSPATH}mgun"':' /usr/app/lib/v2ray/configgun.json.template > /usr/app/lib/v2ray/v2raym.gun.json
